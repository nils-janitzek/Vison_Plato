...
class Fig(object):
    ...
    def plot(self, **kwargs):
        ...
        if self.figname=="/lhome/njanitzek/ESTEC/N-analogue_chain_pipeline/VISON/vison/vison/dark/RESULTS/PLATO/figs/BIAS0X_RON_distro_allOBSIDs.png":    
            print("self.figname:",self.figname)
            print("self.data:",self.data)
            print("\n")
            try:
                print(",shape(self.data),max(isnan),max(isinf),hist(data)",np.shape(self.data),np.amax(np.isnan(np.ravel(self.data))),np.amax(np.isinf(np.ravel(self.data))),np.histogram(np.ravel(self.data),np.arange(np.amin(self.data),np.amax(self.data),(np.amax(self.data)-np.amin(self.data))/10.)))
            except TypeError:
                pass
            #print(TEST)#test_NJ 
