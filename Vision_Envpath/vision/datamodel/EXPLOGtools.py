...
def loadExpLog(expfile, elvis=context.elvis, safe=False):
    ...
    with opener(expfile) as f:
        ...
        explog = ascii.read(lines)
        #explog = ascii.read(lines, data_start=1, delimiter='\t', guess=False,names=columnlist[elvis],format='no_header')
        print("explog,len(explog):", explog,len(explog))
    return explog
