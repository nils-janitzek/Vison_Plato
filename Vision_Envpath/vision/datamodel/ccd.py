...
NcolsCCD = 2255#Euclid-2048,Plato-2255
NrowsCCD = 4509#Euclid-2066,Plato-4509->should be 4510...check!
prescan = 25#51-Euclid,25-Plato 
overscan = 15#20-Euclid,15-Plato
voverscan = 30#20-Euclid,30-Plato#?
#voverscan = 0#?
#NAXIS1 = (prescan + NcolsCCD + overscan) * 2
#NAXIS2 = (NrowsCCD + 20) * 2
NAXIS1 = (prescan + NcolsCCD + overscan)*2
#NAXIS2 = (NrowsCCD)# no overscan here!#?
NAXIS2 = (NrowsCCD)+voverscan
#self.NAXIS1=2295#Plato CCD pixel dimensions from Fits files
#self.NAXIS2=4539#Plato CCD pixel dimensions from Fits files
        

#imgarea = [2048,2066]
RON = 1.4#1.9 for Plato
gain = 3.1#around 21 for Plato  # e/adu

#Plato: pre:25, over:15, vover=30 

QuadBound = dict(E=[NAXIS1//2, NAXIS1, 0, NAXIS2],
                 F=[0, NAXIS1//2, 0, NAXIS2])
...
class CCD(object):
    ...
    def __init__(self, infits=None, extensions=None, getallextensions=False, withpover=True,
        ...
        """
        print("CCD init: self.NrowsCCD",self.NrowsCCD)
        ...
        #self.NAXIS1 = (prescan + NcolsCCD + overscan)
        #if withpover:
        #    self.NAXIS2 = NAXIS2
        #else:
        ...
        #self.NAXIS1=2295#Plato-NJ_workaround_1
        #self.NAXIS2=4539#Plato-NJ_workaround_1
        print("self.NAXIS1,self.NAXIS2:",self.NAXIS1,self.NAXIS2)
        ...
        for iext in range(self.nextensions):
            if self.extensions[iext].data is not None:
                #print("self.extensions",self.extensions[iext].data) 
                print("CCD dim. check:", self.shape, self.extensions[iext].data.shape)
                #assert self.shape == self.extensions[iext].data.shape#change_NJ

        #self.chinjlines = 2#does not exist in Plato CCD geometry
        ...
        self.gain = dict(E=3.1, F=3.1)#check
        self.rn = dict(E=4.5, F=4.5)#check

        self.QuadBound = f_get_QuadBound(self.NAXIS1, self.NAXIS2)

        self.wQphys = self.NAXIS1 // 2 - self.prescan - self.overscan
        self.hQphys = self.NAXIS2
        self.QuadBoundPhys = f_get_QuadBound(2 * self.wQphys, self.hQphys)

        self.masked = False

        self.historial = []

        self.Qmatrix = np.array([['F', 'E']])
...
#                        UR='F')
        ...
        self.xCCD2Physoffsets = dict(F=self.prescan,
                                     E=2 * self.overscan + self.prescan,
                                     )

        self.yCCD2Physoffsets = dict(E=0,
                                     F=0)
    ...
    def cooconv_Qrel_2_Qcan(self, x, y, Q):
        ...
        if Q =='F':
            ...
            xsign = 1.
        if Q =='E':
            xzero = BB[1] - BB[0] - 1.#alternative: xzero=BB[1] (reverse readout of 'E')
            xsign = 1.#xsign=-1
    ...
    def cooconv_Phys_2_CCD(self, x, y):
        ...
        isonSi = ((x >= 0) & (x < self.wQphys * 2) &
                  False == ((y >= self.hQphys) & (y <= self.hQphys - 1)))
        xp, yp = np.nan, np.nan
        if isonSi:
            xp = x + self.xCCD2Physoffsets[Q]
            yp = y + self.yCCD2Physoffsets[Q]
    ...
    def loadfromFITS_original(self, fitsfile, extensions=[-1], getallextensions=False):
        ...
        for iext in extensions:
            ...
            if hdu.data is not None:
                ...
                data = None
            header = hdu.header
            print("FITS data array:",data, np.shape(data))
    ...
    def loadfromFITS(self, fitsfile, extensions=[-1], getallextensions=False):
        ...
        fitsfiles=["DATA/PLATO/CCD_DATA/ID12012_F_image_2_CCD2_12_21_13.fits","DATA/PLATO/CCD_DATA/ID12010_E_image_1_CCD1_12_04_41.fits"]
        headers=[]
        datas=[]
        for i,fitsfile in enumerate(fitsfiles):
            ...
            hdulist.close()
        datas=np.vstack(np.array(datas))
        print("datas, shape(datas)",datas,np.shape(datas))        
        self.extensions.append(Extension(datas, header, label))
        self.extnames.append(label)
    ...
    def get_quad(self, Quadrant, canonical=False, extension=-1):
        ...
        print("get_quad: Quadrant,edges,shape(data):",Quadrant,edges,np.shape(self.extensions[extension].data)) 
        Qdata = self.extensions[extension].data[edges[0]:edges[1], edges[2]:edges[3]]
        print("get_quad: shape(Qdata):",np.shape(Qdata))
        if canonical:
            if Quadrant == 'F':
                Qdata = Qdata[:, ::-1].copy()#?
            elif Quadrant == 'E':
                Qdata = Qdata[::-1, ::-1].copy()#?
        print("get_quad-2: shape(Qdata):",np.shape(Qdata))
        return Qdata
    ...
    def get_stats(self, Quadrant, sector='img', statkeys=None, trimscan=None,
                  ...
                  clip=None):
        """ """
        print("get_stats: VEND, NrowsCCD, voverscan", VEND, NrowsCCD, voverscan)

        Qdata = self.get_quad(Quadrant, canonical=True, extension=extension)
        print("Quadrant, Qdata, shape(Qdata):", Quadrant, Qdata, np.shape(Qdata))    
        ...
        print("trimscan",trimscan)
        print("sector, hlims[0],hlims[1], vlims[0],vlims[1]:",sector, hlims[0],hlims[1], vlims[0],vlims[1]) 

        if clip is not None:
            vals = stats.sigmaclip(vals, clip[0], clip[1]).clipped

        print("stat: vals, shape(vals)",vals,np.shape(vals))
