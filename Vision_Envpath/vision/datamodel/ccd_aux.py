...
def extract_region(ccdobj, Q, area='img', vstart=0, vend=2086,
    ...
    print("imgx",imgx,ccdobj.NAXIS1,prescan,overscan)
    ...
    print("Qdata,np.shape(Qdata),area:",Qdata,np.shape(Qdata),area) 
...
class Model2D():
    ...
    def __init__(self, img, corners=None):
        ...
        print("Model2D, img.shape",img.shape)
...
def get_region2Dmodel(ccdobj, Q, area='img', kind='spline', splinemethod='cubic', pdegree=2,
    ...
    subregion, BB = ccdobj.extract_region(Q, area=area, vstart=vstart, vend=vend,
                                          ...
                                          extension=extension)
    print("subregion, shape(subregion) BB",subregion, np.shape(subregion), BB,Q,area,vstart,vend)
    regmodel = Model2D(subregion)
    #print("subregion,shape(subregion),regmodel",subregion,np.shape(subregion),regmodel)
...
def get_1Dprofile(ccdobj, Q, orient='hor', area='img', stacker='mean', vstart=0,
    ...
    prescan = ccdobj.prescan
    overscan = ccdobj.overscan
    imgx = ccdobj.NAXIS1 - prescan - overscan#?
    ...
    if orient == 'hor':
        ...
        if Q in ['G', 'H']:
            ...
            x = np.arange(BB[3], BB[2], -1) + ccdobj.NAXIS2
    v1d = Profile1D(x, y)
