...
def check_test_structure(explog, structure, CCDs=[1, 2, 3, 4], selbool=True, wavedkeys=[]):
...
def addHK(dd, HKKeys, elvis=context.elvis):
    ...
    filetype_extension="HK_DATA/"
    
    HKlist = []

    for iOBS, ObsID in enumerate(ObsIDs):

        tmp = os.path.join(datapaths[iOBS], filetype_extension, 'HK_%s_*_ROE1.txt' % ObsID)
        print("HK path+file",tmp)
        HKs = glob(tmp)
