    ...
    if not self.drill:
        ...
        for iObs in range(nObs):
            for jCCD, CCDk in enumerate(CCDs):
                filetype_extension="CCD_DATA/"
                dpath = self.dd.mx['datapath'][iObs, jCCD]+filetype_extension
...
def check_metrics_T(self):
    ...
    if self.inputs['test'] != 'PERSIST01':
        ...
        NQactive = np.repeat(NQactive[..., np.newaxis], len(Quads), axis=-1)  # extend to Quads
